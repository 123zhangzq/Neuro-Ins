...
class PDPTW(object):
    ...
    NAME = 'pdptw'  #Pickup and Delivery with Time Window
    ...
    def __init__(self, p_size, sta_orders, init_val_met = 'p2d', with_assert = False):
        ...
        self.state = 'eval'
        print(f'PDPTW with {self.size} nodes.',
              ' Do assert:', with_assert,)
    
    def input_feature_encoding(self, batch):
        aa = torch.cat([batch['coordinates'], batch['dynamic_loc']], dim=1)
        aa[:, :, 2] = aa[:, :, 2] / 10
        aa[:, :, 3] = aa[:, :, 3]/10
        aa[:, 0, 3] = 1
        # aa = torch.cat([aa[:, :, :2], aa[:, :, 3:]], dim=2)
        return aa
    ...
    def get_costs(self, batch, rec, flag_finish=False):
        ...
        all_coor = torch.cat([batch['coordinates'], batch['dynamic_loc']], dim=1)[:,:,:2]
        all_left_tw = torch.cat([batch['coordinates'], batch['dynamic_loc']], dim=1)[:, :, -2]
        all_right_tw = torch.cat([batch['coordinates'], batch['dynamic_loc']], dim=1)[:, :, -1]
        ...
        overtime = torch.zeros(batch_size).to(rec.device)
        total_length = torch.zeros(batch_size).to(rec.device)
        curr_node = torch.zeros(batch_size).to(rec.device)
        curr_time = torch.zeros(batch_size).to(rec.device)
        for i in range(size-1):
            index = curr_node.long().unsqueeze(1).unsqueeze(2).expand(-1, 1, batch['dis_matrix'].size(1))
            curr_mx = batch['dis_matrix'].gather(1, index).squeeze(1)
            index2 = rec.gather(1, curr_node.long().unsqueeze(-1))
            if index2[0][0] == 0:
                break
            dis = curr_mx.gather(1, index2).squeeze(1)
            curr_time += dis
            left_tw = all_left_tw.gather(1,index2).squeeze(1)
            right_tw = all_right_tw.gather(1,index2).squeeze(1)
            curr_time = torch.max(curr_time,left_tw)
            delay = torch.clamp((curr_time - right_tw), min=0)
            overtime += delay
            total_length += dis
            curr_node = index2.squeeze(1)

        return total_length + overtime



        # if size == 2 * self.static_orders + 1:
        #     # calculate obj value
        #     d1 = batch['coordinates'][:,:,:2].gather(1, rec.long().unsqueeze(-1).expand(batch_size, size, 2))
        #     d2 = batch['coordinates'][:,:,:2].clone()
...
class PDPDataset(Dataset):
    def __init__(self, filename=None, size=20, num_samples=10000, offset=0, distribution=None, flag_val=False):
        ...
        for i, instance in enumerate(self.data):
            self.data[i]['coordinates'] = torch.cat((instance['depot'].reshape(1, 4), instance['loc']),dim=0)
            del self.data[i]['depot']
            del self.data[i]['loc']
            all_coor = torch.cat([self.data[i]['coordinates'], self.data[i]['dynamic_loc']], dim=0)[:, :2]
            self.data[i]['dis_matrix'] = self.calculate_distance(all_coor)
