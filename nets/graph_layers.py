...
class MultiHeadAttentionNew(nn.Module):
    ...
    def apply_rotary_emb(self,
        ...
        # Convert to plural domain
        xq_ = torch.view_as_complex(xq_)
        xk_ = torch.view_as_complex(xk_)

        # Apply a rotation operation and convert the result back to the real domain
    ...
    def forward(self, h, out_source_attn):
        ...
        # Apply rotational position encoding before attention
...
class EmbeddingNet(nn.Module):
    ...
    def get_visited_time(self, solutions, step_info):
        ...
        valid_half_size = valid_seq_length // 2
    ...
    def precompute_freqs_cis(self, dim: int, index, theta: float = 10000.0):
        # Generate token sequence index t = [0, 1,..., seq_len-1]
